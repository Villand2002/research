...
class Outcome:
    ...
    def _category_allocations(self, dataset: Dataset) -> Dict[int, list]:
...
if __name__ == "__main__":
    dataset = Dataset.build(dataset_id=1, num_agents=4, num_categories=3, seed=7)
    sample_matching = {0: 0, 1: 1, 2: 2}  # agent_id -> category_id
    outcome = Outcome(dataset_id=dataset.id, algorithm_name="Example", matching=sample_matching)

    print("Feasible:", outcome.verify_feasible(dataset))
    print("Fairness:", outcome.verify_fairness(dataset))
    print("Non-wasteful:", outcome.verify_nonwasteful(dataset))
    print("Stability:", outcome.verify_stability(dataset))
